...
class PasswordController extends Controller
{
    public function forgotPassword(ForgotPasswordRequest $request)
    {
        ...
        if ( !$user )
        {
            return commonErrorMessage("Invalid Email", 404);
        }
        ...
        return apiSuccessMessage('Password reset otp has been sent to your email address',['id'=> $user->id]);
    ...
    public function resetForgotPassword(ResetForgotPasswordRequest $request)
    {
        ...
        if ( !$check_otp )
        {
            ...
            $user->save();
            $check_otp->delete();
            return commonSuccessMessage("Password Updated");
    }
}
